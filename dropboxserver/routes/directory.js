...
exports.createDirectory = function(req, res) {
    ...
    let jsonResponse = {};
        ...
        let createDir = "insert into directory(dir_uuid,dir_name,dir_created,user_uuid,hasFiles) values ('"
            + uuidv4 + "','" + req.body.dir_name + "','" + dir_created_timestamp + "','" + req.body.user_uuid + "', '0');";
...
exports.starDir_files = function(req, res) {
    ...
    mysqlConnection.userSignup(starQuery, function (err, result) {
        if (err) {
            ...
            if (result.affectedRows > 0) {
                mysqlConnection.userSignup(mapping_star, function (err, result1) {
                    if (result1.affectedRows > 0) {
...
exports.getFiles = function(req, res) {
    ...
    var filesArray =[];
    let getFilesQuery = " select f.file_name, f.file_uuid, f.file, f.file_created from file f where f.user_uuid = '" + sessionMgmt.user_uuid +
    ...
    var fileDirArray =[];
    let fileDirQuery = "select fdr.user_uuid, fdr.dir_uuid, d.dir_created, d.dir_name, f.file_name, fdr.file_uuid, f.file," +
        " f.file_created, fdr.star_id from file_dir_user fdr left join file f on f.file_uuid = fdr.file_uuid" +
